...
def build_awards_embed():
    ...
    for s in schedule:
        ...
        if h_pts != a_pts:
            if h_pts > a_pts:
                results.append((h_id, a_id, h_pts - a_pts))
            else:
                results.append((a_id, h_id, a_pts - h_pts))
    ...
    if high:
        embed["fields"].append({"name":"👑 High score 👑", "value": f"{team_display(teams[high[0]])} with {high[1]:.2f} points", "inline": False})
    if low:
        embed["fields"].append({"name":"💩 Low score 💩",  "value": f"{team_display(teams[low[0]])} with {low[1]:.2f} points", "inline": False})
    if blow:
        embed["fields"].append({"name":"😱 Blow out 😱",   "value": f"{team_display(teams[blow[0]])} blew out {team_display(teams[blow[1]])} by {blow[2]:.2f} points", "inline": False})
    if close:
        embed["fields"].append({"name":"😅 Close win 😅",  "value": f"{team_display(teams[close[0]])} barely beat {team_display(teams[close[1]])} by {close[2]:.2f} points", "inline": False})

    if not embed["fields"]:
        embed["description"] = "_No results recorded for the prior week yet_"
